...
import { BarChart, PieChart, TrendingUp, Calendar, Clock, Landmark, Activity, Compass } from 'lucide-react';

export const Stats: React.FC = () => {
  const { books, sessions, notes } = useStore();
  ...
  const formatCounts = books.reduce((acc, book) => {
    ...
    return acc;
  }, { physical: 0, ebook: 0, audiobook: 0, library: 0 } as Record<Book['format'], number>);
  ...
  // 3. Weekly Pages Data (Mocking past 4 weeks)
  const weeklyPages = [340, 210, 480, totalPagesRead % 500]; // Simulated weekly trend
  ...
  // Heatmap sessions grid (May 2026)
  const renderHeatmap = () => {
    // Weeks rows (5 weeks in May)
    const weeks = [];
    const readDays = new Set(sessions.map(s => new Date(s.start_time).getDate()));
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      {/* Header */}
      <div>
        <h1 className="screen-title">Dashboards</h1>
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid var(--border-glass)', gap: '1.5rem', paddingBottom: '0.1rem' }}>
        {[
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '3rem', alignItems: 'center' }}>
            <div>
              ...
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '1.5rem' }}>
                Your page completions mapped over the last 4 weeks. Consistently reading 20-30 pages a day!
              </p>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div style={{ background: 'rgba(255,255,255,0.01)', border: '1px solid var(--border-glass)', padding: '1rem', borderRadius: 'var(--radius-md)' }}>
                  ...
                  <div style={{ fontSize: '1.5rem', fontWeight: 800, marginTop: '0.25rem' }}>{totalPagesRead} pages</div>
                </div>
                <div style={{ background: 'rgba(255,255,255,0.01)', border: '1px solid var(--border-glass)', padding: '1rem', borderRadius: 'var(--radius-md)' }}>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Time Spent</div>
                  <div style={{ fontSize: '1.5rem', fontWeight: 800, marginTop: '0.25rem' }}>{totalHours} hours</div>
                </div>
              </div>
            </div>
        ...
        {activeSubTab === 'formats' && (
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '3rem', alignItems: 'center' }}>
            <div>
              ...
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '1.5rem' }}>
                Your library format breakdown. Ebooks lead the list, followed by Physical copies and borrowed Library loans.
              </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <div>
              ...
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
                Where and when you log the most sessions. Cafe sessions reflect high focus levels!
              </p>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3rem' }}>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <div className="flex-between" style={{ padding: '0.75rem', background: 'rgba(255,255,255,0.01)', border: '1px solid var(--border-glass)', borderRadius: '6px' }}>
                  <span>Captured Notes / Highlights</span>
        ...
        {activeSubTab === 'time' && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '3rem', alignItems: 'center' }}>
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div style={{ background: 'rgba(255,255,255,0.01)', border: '1px solid var(--border-glass)', padding: '1rem', borderRadius: 'var(--radius-md)' }}>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Active Days (May)</div>
                  <div style={{ fontSize: '1.5rem', fontWeight: 800, marginTop: '0.25rem', color: 'var(--accent-primary)' }}>
                    {new Set(sessions.map(s => new Date(s.start_time).getDate())).size} days
                  </div>
                </div>
                <div style={{ background: 'rgba(255,255,255,0.01)', border: '1px solid var(--border-glass)', padding: '1rem', borderRadius: 'var(--radius-md)' }}>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Total Sessions</div>
                  <div style={{ fontSize: '1.5rem', fontWeight: 800, marginTop: '0.25rem' }}>{sessions.length}</div>
                </div>
              </div>
            </div>
