...
import { Play, Sparkles, BookOpen, Quote, ChevronRight, Users, Star, Landmark } from 'lucide-react';
import { format } from 'date-fns';

export const Home: React.FC = () => {
  const { 
    books, sessions, notes, circles, activeSession, startReadingSession, setScreen, setSelectedBookIdForDetail 
  } = useStore();

  const readingBooks = books.filter(b => b.status === 'reading');
  const finishedBooks = books.filter(b => b.status === 'finished');
  const favoriteNotes = notes.filter(n => n.is_favorite);
  const latestNotes = [...notes].sort((a,b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).slice(0, 3);
  ...
  const calculateStreak = () => {
    ...
    let expectedDate = new Date(firstSessionDate);
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 className="screen-title" style={{ fontSize: '2.2rem' }}>Welcome back, Rithu</h1>
      ...
      <div className="bento-grid">
        ...
        <div className="glass bento-card bento-col-4" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: '160px' }}>
          ...
          <div style={{ margin: '1rem 0' }}>
            ...
            <span style={{ fontSize: '1.1rem', color: 'var(--text-secondary)' }}> / 12</span>
            <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)', display: 'block', marginTop: '0.25rem' }}>books completed</span>
          </div>
          <div>
            <div className="progress-bar-container" style={{ height: '4px' }}>
              <div className="progress-bar-fill" style={{ width: `${Math.min(100, (finishedBooks.length / 12) * 100)}%` }}></div>
            </div>
            <div className="flex-between" style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '0.35rem' }}>
              <span>{Math.round((finishedBooks.length / 12) * 100)}% complete</span>
              <span>On track</span>
            </div>
          </div>
        </div>
        ...
        <div className="glass bento-card bento-col-4" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: '160px' }}>
          ...
          <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
            Average session length: {sessions.length > 0 ? Math.round(sessions.reduce((acc, s) => acc + s.duration_minutes, 0) / sessions.length) : 0} minutes.
          </p>
        </div>

        {/* Currently Reading (Bento grid column span 8) */}
        <div className="bento-card bento-col-8" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', marginTop: '1rem' }}>
          <div className="flex-between">
            ...
            <button className="btn btn-text" onClick={() => setScreen('shelf')} style={{ display: 'flex', alignItems: 'center', fontSize: '0.8rem', padding: 0 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {readingBooks.length === 0 ? (
              <div className="glass" style={{ padding: '2rem', textAlign: 'center' }}>
                ...
                <button className="btn btn-primary" style={{ color: '#091A1E', fontWeight: 700 }} onClick={() => setScreen('shelf')}>Browse To Read list</button>
              </div>
            ) : (
              readingBooks.map(book => {
                const isTimerActive = activeSession?.bookId === book.id;
                return (
                  <div key={book.id} className="glass book-card-3d" style={{ padding: '1rem', display: 'flex', gap: '1.25rem', alignItems: 'center', transition: 'border-color 0.2s', overflow: 'visible' }} onClick={() => setSelectedBookIdForDetail(book.id)}>
                    {/* 3D Tactile Cover */}
                    ...
                    <div style={{ flexShrink: 0 }}>
                      {isTimerActive ? (
                        <button 
                          ...
                          onClick={(e) => { e.stopPropagation(); setScreen('track'); }}
                        >
                          Active
                        </button>
                      ) : (
        ...
        {/* Right Column: Quotes & Circles (Bento grid column span 4) */}
        <div className="bento-card bento-col-4" style={{ display: 'flex', flexDirection: 'column', gap: '2rem', marginTop: '1rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div className="flex-between">
              ...
              <button className="btn btn-text" onClick={() => setScreen('remember')} style={{ padding: 0, fontSize: '0.8rem' }}>
                All Notes
              </button>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {latestNotes.length === 0 ? (
                <div className="glass" style={{ padding: '1.5rem', textAlign: 'center', color: 'var(--text-muted)' }}>
                  ...
                  <p style={{ fontSize: '0.85rem' }}>No highlights captured yet. Double click notes to add tags.</p>
                </div>
              ) : (
                    ...
                    <div key={note.id} className="glass" style={{ padding: '1rem', background: 'rgba(255,255,255,0.01)', borderLeft: '3px solid var(--accent-primary)' }}>
                      ...
                      <div className="flex-between" style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '0.5rem' }}>
                        ...
                        <span style={{ display: 'flex', alignItems: 'center', gap: '0.2rem' }}>
                          {note.is_favorite && <Star size={10} fill="var(--color-on-hold)" style={{ color: 'var(--color-on-hold)' }} />}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div className="flex-between">
              ...
              <button className="btn btn-text" onClick={() => setScreen('groups')} style={{ padding: 0, fontSize: '0.8rem' }}>
                View Circles
              </button>
            </div>
                  ...
                  <div key={circle.id} className="glass" style={{ padding: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      ...
                      <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '0.15rem' }}>
                        👥 {circle.member_count} members • {circle.threads.length} topics
                      </p>
                    </div>
                    <button className="btn btn-secondary" style={{ padding: '0.3rem 0.6rem', fontSize: '0.75rem' }} onClick={() => setScreen('groups')}>
                      Open
                    </button>
                  </div>
                ))
